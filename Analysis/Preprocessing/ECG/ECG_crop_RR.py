...
if __name__ == "__main__":
    ...
    for subject in range(1, nSub+1):
        ...
        if os.path.isfile(wdic + file_name):
            ...
            # concat_hrv_mat['Res'][0][0][3][0][0] + [0]Param, [1]Data, [2]Statistics,
            #                                        [3]Frequency, [4]NonLinear
            T_R = concat_hrv_mat['Res'][0][0][3][0][0][1][0][0][1]
            # Data + [0][0] + [0]Rsgn, [1]T_R, [2]RR ...
            ...
            if len(kick_out) > 0:
                ...
                fill_word = ["entry", "was", "its"] if len(kick_out) == 1 else ["entries",
                                                                                "were",
                                                                                "their"]
                log_file.append("Subject_{} \n".format(subject))
                log_file.append("__T_R & RR: {} {}, which {} too close to {} neighbour was kicked out – "
                                "at index{} \n".format(len(kick_out), fill_word[0], fill_word[1],
                                                       fill_word[2], kick_out))
            ...
            if int(time_table[subject-1, 0]) == subject:
                ...
                for t_point in T_R:
                    ...
                    if ind_01 < len(vector_01):
                        ...
                        log_file.append("____Try to set 1 in vector_01 (len={}) beyond bounds (at idx={})"
                                        ", T_R is {} sec too long\n".format(
                            len(vector_01), ind_01, (ind_01 - len(vector_01)) / s_freq))
                ...
                for phase in phases:
                    ...
                    file_name_crop_v01_trim = "NVR_S{}_{}_T_R_v01.txt".format(str(subject).zfill(2),
                                                                              phase)
                    ...
                    if not no_entries:
                        ...
                        # 1) Time/samp.-freq, time point to cut:
                        # t_cut += (time_table[subject-1, phase_count]+1)/s_freq
                        # # 2) init. 0/1 vector: vector_01
                        ...
                        if len(T_R) > t_crop_count+1:  # For last phase not applicable
                            otcc = old_t_crop_count
                            if not (T_R[otcc:t_crop_count][-1] < t_cut <= T_R[otcc:t_crop_count + 1][-1]):
                                print("S_{} | Phase: {}, Wrong cut at T_R".format(subject, phase))
                                print("Last value: {}sec".format(T_R[otcc:t_crop_count][-1]))
                                print("Time to cut: {}sec".format(t_cut))
                                print("Next value: {}sec".format(T_R[otcc:t_crop_count+1][-1]))
                        ...
                        m_HR_df[subject-1, phase_count] = (len(crop_phase)*60)/(
                                time_table[subject-1, phase_count]/s_freq)
    ...
    for file in os.listdir(wdic_cropTR):
        if "Space" in file or "Ande" in file:
            trimmed_time = trimmed_time_space if "Space" in file else trimmed_time_ande
            # if "Ande" in file
