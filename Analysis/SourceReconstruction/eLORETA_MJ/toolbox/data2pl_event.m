function [cs,nave]=data2pl_event(data,segleng,segshift,epleng,maxfreqbin,para);
% usage: [cs,nave]=data2pl_event(data,segleng,segshift,epleng,maxfreqbin,para)
% 
...
proj=[];
 ...
 maxfreqbin=floor(min([maxfreqbin,floor(segleng/2)+1])/order);
[ndum,npat]=size(proj);
    ...
    for i=1:nseg; %average over all segments;
        ...
        if mydetrend==1
         ...
         for f=1:maxfreqbin % for all frequencies
          if npat==0
             if segave==0
                 cs(:,:,f,i)=cs(:,:,f,i)+conj((datalocfft(f,:).^order)'*datalocfft(order*(f-1)+1,:)); 
		 av(:,f,i)=av(:,f,i)+conj(datalocfft(f,:)');
             else 
                %disp([i,f,size(datalocfft)])
                cs(:,:,f)=cs(:,:,f)+conj((datalocfft(f,:).^order)'*datalocfft(order*(f-1)+1,:)); 
		av(:,f)=av(:,f)+conj(datalocfft(f,:)');
             end
          else 
             if segave==0
                 cs(:,:,f,j,i)=conj((datalocfft(f,:).^order)'*datalocfft(order*(f-1)+1,:));
                 av(:,f,j,i)=conj(datalocfft(f,:)');  
             else 
                %disp([i,f,size(datalocfft)])
                cs(:,:,f,j)=cs(:,:,f,j)+conj((datalocfft(f,:).^order)'*datalocfft(order*(f-1)+1,:));
