function basis=legs(x,n)
% usage: [basis,gradbasis]=legs(x,dir,n)
%
% returns the values and directional derivatives  of (n+1)^2 
% spherical harmonics at locations given in x and, for the 
...
%
% output: basis: Nx((n+1)^2  matrix containing in the j.th  row the real 
%                and imaginary parts of r^kY_{kl}(theta,Phi)/(N_{kl}*scale^k) ( (r,theta,phi) 
%                are the spherical coordinates corresponding to  the j.th
%                row in x) %                for k=0 to n and l=0 to k 
%                the order is:
%                          real part for k=0;
%                  then    real parts for k=1 and l=0,1 (2 terms) 
...
%
%         gradbasis: Nx(n+1)^2 matrix containing in the j.th row the scalar 
...
for i=1:n
    for j=1:i+1
        if j==1
            normalize(i,j)=sqrt(2*facto(i+j-1+1)/facto(i-j+1+1)/(2*i+1));
        else 
            normalize(i,j)=sqrt(facto(i+j-1+1)/facto(i-j+1+1)/(2*i+1));
        end
    end
end
...
% 
% rad0=reshape(repmat((repmat(rad,1,n))',n+1,1),n,n+1,n1);
% nss=reshape(repmat(repmat(ns,n1,1)',n+1,1),n,n+1,n1);
% leg0_b=leg0.*(rad0.^nss);
%      onesx=zeros(1,n+1,n1);
%      onesx(1,1,:)=ones(1,1,n1);
%      
% legshift=[onesx;leg0_b(1:n-1,:,:)];
...
%     
% rdotdir= reshape(repmat((x(:,1).*dir(:,1)+x(:,2).*dir(:,2)+x(:,3).*dir(:,3))',n*(n+1),1),n,n+1,n1);
% gradleg=(gradleg./(rad0.^nss)-nss.*leg0./(rad0.^2).*rdotdir)./normalize;
leg0=leg0./normalize;
 
    basis=zeros(n1,(n+1)^2-1);
    %gradbasis=zeros(n1,(n+1)^2-1);
 for i=1:n
...
%      gradbasis(:,i^2+i+1:(i+1)^2-1)=(reshape(imag(gradleg(i,2:i+1,:)),i,n1))';    
     ...
     basis=[ones(n1,1)/sqrt(2),basis];
%      gradbasis=[zeros(n1,1),gradbasis];
%  
