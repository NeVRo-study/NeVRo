...
% diagonalizes matrices stored in M(:,:,f) for all f
% simplest version:
% Input: M NxNxM tensor with N channels and M matrcies to be 
%          diagonalized
...
%              regarded as vectors.)  
%        para.backweight  weight given to previous gradients 
%                         (default 0) (that was just an idea)
% Output: v:     demixing matrix, such that v'*M(:,:,f)*v 
%                is approximately diagonal
%         diags: N-M matrix of diagonals for N ISA-components 
%                and for M 'frequencies'. 
%         parout: right it contains only a field named err which
