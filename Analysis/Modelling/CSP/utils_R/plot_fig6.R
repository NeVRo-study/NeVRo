...
## Contains the functions that each plot a subfigures of Figure 6 
## in the paper.
...
plot_fig6_csp <- function(df_full, plt, path_out, show_legend = FALSE) {

  ## Plot the 2x2 topoplot subfigure in Figure 6 in the paper. 
...
plot_fig6_spoc <- function(df_full, plt, path_out, save_png = TRUE) {
  
  ## Plot the 1x2 SPoC topoplot subfigure in Figure 6 in the paper. 
  ## Takes the data frame w/ all normalized (!) pattern weights, 
...
plot_fig6_ssd <- function(df_full, plt, path_out, save_png = TRUE) {
  
  ## Plot the 4x2 SSD topoplot subfigure in Figure 6 in the paper. 
  ## Takes the data frame w/ all normalized (!) pattern weights, 
...
plot_fig6_supp <- function(df_full, cond, plt, path_out, save_png = TRUE) {
  
  ## Plot the supplements for Figure 6 in the paper. 
  ## Takes the data frame w/ all normalized (!) pattern weights, 
  ...
  if (save_png) {
    fname <- str_glue('fig6_supp_{cond}.png')
    ggsave(file = file.path(path_out, fname), 
           plot = gt, 
           device = 'png', 
           width = unit(w, 'cm'), #unit(plt$wid_heigth_spoc[1], 'cm'), 
