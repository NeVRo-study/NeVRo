...
def sort_table(task, table=None, table_path=None):
    ...
    if table is None:
        ...
        assert table_path, "path to table file (.csv) must be given. This assumes that table lies in " \
                           "subfolder of '/processed/Random_Search_Tables/'"
        ...
        if os.path.isfile(table_path):
            ...
            raise FileNotFoundError(f"No table given & no table found in "
                                    f"'./processed/Random_Search_Tables/'")
    ...
    if save_externally:
        np.savetxt(fname="." + table_path.split(".")[1] + "_sorted.csv", X=sorted_table, delimiter=";",
                   fmt="%s")
    else:
        return sorted_table
...
def merge_randsearch_tables(task, condition, search, sort=True):
    ...
    # Find tables
    wd_table = f"./processed/Random_Search_Tables/{cond}/{0 if search == 'broad' else 1}_{search}_" \
               f"search/{task}/"
    ...
    if len(list_of_tables) <= 1:
        ...
        if len(list_of_tables) == 1:
            if "sorted" in list_of_tables[0]:
                ...
                sort = ask_true_false(f"Do you want to sort this one table "
                                      f"'{wd_table+list_of_tables[0]}'?", 'b')
            if sort:
        ...
        if ask_true_false("Do you want to merge these tables:", col='b'):
            ...
            for table_name in list_of_tables[1:]:
                ...
                if not np.all(pull_table[:, :-3] == rs_table[:, :-3]):
                    raise ValueError("Except of results, tables must be same in order to merge them.")
                    ...
                    for tkey in tab_dict:
                        if tab_dict[tkey][row, -3] != "nan":
                            if not val_found:
                                ...
                                # ... fill information from another table
                                val_found = True
                                ...
                                if ask_true_false("Do you want to stop merging to inspect tables", 'y'):
    ...
    # Save file
    export_filename = prime_tablename.split(".csv")[0] + ("_merged" if merge else "") + (
        "_sorted" if sort else "")
    np.savetxt(fname=export_filename+".csv", X=rs_table, delimiter=";", fmt="%s")

    return export_filename.split(f"{task}/")[1] + ".csv"  # e.g. Random_Search_Table_Reg_merged_sorted.csv
...
def table_per_subject(table_name, condition, search):
    ...
    wd_tables = f"./processed/Random_Search_Tables/{cond}/{0 if search == 'broad' else 1}_{search}_" \
                f"search/{task}/"
    ...
    for sub in subjects:
        ...
        export_filename += table_name.split(tpfix)[1].split(
            "_m" if "merged" in table_name else "_s")[0] + ".csv"
...
def table_per_hp_setting(table_name, condition, search, fixed_comps=False):
    ...
    :param condition: "mov" or "nomov"
    :param search: 'broad' OR 'narrow'
    :param fixed_comps: True: selection of components is identical for each subj. per hyper-parameter set;
                        False: individual comps per subject, selected under same rules
                            ('random' OR 'one-up') and same N
    """
    condition = condition.lower()
    assert "mov" in condition, "condition must be either 'mov' or 'nomov'"
    cond = "nomov" if "no" in condition else "mov"
    ...
    if fixed_comps:
        for setx, setting in enumerate(hp_settings):
            # Extract sub-table
            set_rs_table = rs_table[np.where(rs_table[:, hp_set_col] == setting)[0], :]
        ...
        # Remove individual comp. information (per subject) from list of unique hyper-parameter settings
...
def table_of_best_hp_over_all_subjects(n, task, condition, search, fixed_comps=False):
    ...
    :param condition: 'mov' OR 'nomov'
    :param search: 'broad' OR 'narrow'
    :param fixed_comps: True: selection of comps is identical for each subject per hyper-parameter set;
                        False: individual comps per subject, selected under same rules ('random' OR
                            'one-up') and same N
    """
    ...
    # Set path
    wd_sub_tables = f"./processed/Random_Search_Tables/{cond}/{0 if search == 'broad' else 1}_{search}_" \
                    f"search/{task}/per_subject/"
    ...
    # Del redundant entries, i.e. 2 or more subjects share n-m best hyperparameter sets, where m in [0,n]
    if fixed_comps:
        ...
        for ridx, row in enumerate(bhp_table_unique_wo_comps[1:, idx_wo_comp_path]):
            ...
            for row_full in bhp_table_unique[1:, :]:
                if np.all(row == row_full[idx_wo_comp_path]):  # check if rows match (without comp-info)
            ...
            row_path = row_path.split("_comp")[0] + f"_comp-nmax-{n_max_comp}" \
                                                    f"_hrc" + row_path.split("_hrc")[1]
    ...
    print(f"Number of unique hyperparameter sets: {bhp_table_unique.shape[0]} (out "
          f"of {bhp_table.shape[0]})")

    # Save
    tpfix = "Ran" if search == "broad" else "Nar"  # broad search: 'Ran' OR narrow search: Nar
    export_filename = f"Best_{n}_HPsets_over_{cntr}_" \
                      f"Subjects_mean_acc_{mc:.3f}_{tpfix}" + exp_filename.split(tpfix)[1]
...
def model_performance(over, task, condition, search, input_type):
    ...
    if over == "subjects":
        ...
        for file_name in os.listdir(wd_tables):
            if file_name[0] == "S" and ".csv" in file_name:
                ...
                if count_entries == 1:
                    ...
                    head_idx += [-1] if task == "classification" else [-3, -2, -1]
                    # [1, 23,( 24, 25,) 26] : accuracies
        ...
        fin_table = np.concatenate((fin_table, np.reshape(
            np.array(["all", "average_performance"] + list(mean_perform.astype(str))),
        ...
        for file_name in os.listdir(wd_tables):
            if "Set" in file_name and ".csv" in file_name:
                ...
                if count_entries == 1:
                    ...
                    fin_table = np.reshape(
                        np.concatenate((["subjects"], rs_table[0, head_idx], ["SD"])),  # add SD col
...
if __name__ == "__main__":
    ...
    d_type = "SSD" if ask_true_false("\nIs 'SSD' the data type? False automatically assumes 'SPoC'",
                                     'y') else 'SPOC'
    ...
    for searchi in searches[scr]:
        for taski in tasks[tsk]:
            for condi in conditions[cnd]:
                ...
                cprint(f"\nExtract results from {searchi} random search in {condi} condition ({taski}) "
                       f"...", 'b')

                # For both conds: 'mov' & 'nomov' AND for both tasks: 'classification' & 'regression' do:
                ...
                table_per_hp_setting(table_name=tab_name, condition=condi, search=searchi,
                                     fixed_comps=fxcmp)

                # 3) Then create table of best HPs over all subjects
                table_of_best_hp_over_all_subjects(n=2, task=taski, condition=condi, search=searchi,
                                                   fixed_comps=fxcmp)

                # 4) Calculate model performance
                # (Has to be done after "narrow"-search. Can be done on 'broad'-random search)
                model_performance(over="subjects", task=taski, condition=condi, search=searchi,
                                  input_type=d_type)
                model_performance(over="hpsets", task=taski, condition=condi, search=searchi,
                                  input_type=d_type)

                # 5) After broad-search, do narrow-search:
                if searchi == "broad":
