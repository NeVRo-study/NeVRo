...
class NeVRoNet:
        ...
        with tf.variable_scope('NeVRoNet'):
            ...
            for layer in range(len(self.lstm_size)):
                ...
                if len(self.lstm_size) > 1 and not last_layer:
                    ...
                    print(f"lstm_input.shape to layer lstm{layer+2} after transpose: {lstm_input.shape}\n")
    ...
    def _create_lstm_layer(self, x, layer_name, lstm_size, last_layer):
        ...
        with tf.variable_scope(layer_name):
            ...
            print(f"\nx in {layer_name}: {x}")  # TEST
            # if "1" in layer_name:
            #     x = tf.transpose(x, [1, 0, 2])  # [250, batch_size, 1]
            #     print(f"\nx in {layer_name} after transpose: {x}")  # TEST
            ...
            print(f"self.lstm_output of layer {layer_name}: {self.lstm_output}\n")  # TEST
            ...
            with tf.name_scope(layer_name + "_elu"):  # or relu
                ...
                print(f"pre_activation of layer {layer_name}: {pre_activation}")  # TEST

                post_activation = self.activation_function(features=pre_activation,
                                                           name="post_activation")

                print(f"post_activation.shape of layer {layer_name}: {post_activation.shape}\n")  # TEST
