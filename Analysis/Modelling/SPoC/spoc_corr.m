function [lambda_samples_end, z_shuff, W_perm, R, lambda_samples, lambda_samples_sign, r_samples,r_samples_sign, W, A, lambda_values, p_values_lambda, Cxx, Cxxz, Cxxe] = spoc_corr(X, z, varargin)
% Source Power Co-modulation Analysis (SPoC). - Variation with permutations
% based on r instead of lambda. 
%
...
if n_bootstrapping_iterations > 0
    ...
    r_samples_sign = zeros(1, n_bootstrapping_iterations);
...
%         z_shuffled = z(randperm(length(z)));
        ...
        W_s = M'*W_s; 
        W_perm{k} = W_s;
        z_shuff{k} = z_shuffled;
        
        % compute and store lambda and correlation values
        lambda_samples(k) = max(abs(lambda_values_s));
        lambda_samples_end(k) = lambda_values_s(end);
        
        [~,X_l] = max(abs(lambda_values_s));
        lambda_samples_sign(k) = lambda_values_s(X_l);
        %lambda_samples_sign_end(k) = 
        
        fv = get_var_features(W_s, Cxxe);
        R = corrcoef([z_shuffled', fv']);
        r_samples(k) = max(abs(R(1,2:end)));  % absolute max correlation
        
        % NeVo - we're specifically interested
        %r_samples(k) = max(abs(R(1,2:end)));
        
        [~,X_s] = max(abs(R(1,2:end)));
        R_line  = R(1,2:end); 
        r_samples_sign(k) = R_line(X_s);% sign max correlation
        
    end
    
    %neg_samples = lambda_samples(lambda_samples<0);
    
    % compute bootstrapped p-values
    for n=1:n_ev
        p_values_lambda(n) = sum(abs(lambda_samples(:))>=abs(lambda_values(n)))/n_bootstrapping_iterations;
        %p_values_lambda(n) = sum(neg_samples(:)<=(lambda_values(n)))/(n_bootstrapping_iterations-sum(lambda_samples<0));
    end
end