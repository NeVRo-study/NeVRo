...
% and directly via permutation in the script.
...
%for isub = 16  - if you want to run a specific subject
...
% Ratings
if exist([rat_path cond{folder} '/SBA/' fileName '_run_1_alltog_rat_z.txt'], 'file')
    rat_tab=readtable([rat_path cond{folder} '/SBA/' fileName '_run_1_alltog_rat_z.txt']);
else
    rat_tab=readtable([rat_path cond{folder} '/SBA/' fileName '_run_2_alltog_rat_z.txt']);
end
...
%% 2. First approach: get_var_features
...
[~,i_maxabs] = max(abs(lambda_values));

struct_summ(isub).ID = fileName;
struct_summ(isub).lambda = lambda_values(end);
struct_summ(isub).lambda_maxabs = lambda_values(i_maxabs);
...
% Save current figure
saveas(gcf,[spoc_path cond{folder} '/SBA/summaries/SPoC_topo_perm_pear_' cond{folder} '.png']);
...
[stats_r.h,stats_r.p,stats_r.ci,stats_r.stats] = ttest([struct_summ.r],[struct_summ.r_perm_avg]);

[stats_rl.h,stats_rl.p,stats_rl.ci,stats_rl.stats] = ttest([struct_summ.r],[struct_summ.r_perm_avg],'Tail','left');

% Save struct
save([spoc_path cond{folder} '/SBA/summaries/SPOC_groupstats_'  cond{folder} '.mat'], 'stats_r');
...
writetable(table_summ,[spoc_path cond{folder} '/SBA/summaries/_summary_.csv']);  % lambda, p.values and corr. coeff summary
