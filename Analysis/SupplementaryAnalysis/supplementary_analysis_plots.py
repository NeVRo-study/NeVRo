...
p2save = p2results.replace("Tables", "Plots/Supplementary_Analysis/")
...
def sign_asterisk(p: float):
    return "ns" if p > .05 else "*" if (.05 >= p > .01) else "**" if (.01 >= p > .001) else "***"
...
if __name__ == "__main__":
    ...
    for cond in conditions:
        ...
        for sp in span:
            for model in models:
                ...
                t, _p = ttest_ind(a=table[data_col], b=table[perm_data_col])  # results of two-sided
                _p /= 2  # one-sided

                cprint(f"{cond}-condition – {sp}: t_{model}={t:.3f}, p_{model}={_p:.4f}", "b", "bo")
                ...
                ax.annotate(sign_asterisk(p=_p), xy=(x_pos, y_pos + 1.), ha="center", va="bottom")
                ax.annotate('',
    ...
    for model in models:
        ...
        # Create figure
        fig, axs = plt.subplots(2, 2, num=f"{model}", figsize=(10, 6), sharex=True, sharey=False)

        ax_ctn = 0
        for cond in conditions:
            p2tab = os.path.join(p2results,
                                 f"results_across_methods_{cond}_supplementary_analysis.csv")
            table = pd.read_csv(p2tab, index_col="Subject")
            ...
            for sp in span:
                ...
                if ax_ctn < 2:
                    ax.legend()
                    ax.set_title(f"{'incl. break' if sp == 'SBA' else 'w/o break'}")
                    ax.set_xlabel("")
                else:
                    ax.set_xlabel("AUC")
        ...
        # # Save plot
        for fm in ['png', 'pdf']:
            plt.savefig(fname=os.path.join(p2save, f"SA-vs-SBA_classification_{model}.{fm}"), dpi=300,
                        format=fm)
